import { ProcessExecution, ProgressLocation, ProviderResult, Task, TaskProvider, TaskScope, Uri, tasks, window, workspace } from "vscode";
import cp from "child_process";
import * as vscode from 'vscode';
import { homedir } from "os";
import { getWorkspaceRoot, getWorkspaceRootPath } from "./utils/util";
import path from "path";
import { CancellationToken, WorkspaceEdit } from "vscode-languageclient";
...
export class SpringCli {
    ...
    enhanceResponse(uri: Uri, projDescription: string, cwd: string): Thenable<string> {
        const args = [
            "ai",
            "enhance-response",
            "--file",
