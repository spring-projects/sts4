...
public class VectorStoreHandler {
	...
	private static final Logger log = LoggerFactory.getLogger(VectorStoreHandler.class);
		...
		public VectorStoreHandler(SimpleLanguageServer server) {
			this.server = server;
			registerCommands();
		}
		...
		private CompletableFuture<Object> getContext(ExecuteCommandParams params) {
			...
			String question = "How to add JPA to project";
	        List<Document> similarDocuments = this.vectorStore.similaritySearch(SearchRequest.query(question).withTopK(2));
